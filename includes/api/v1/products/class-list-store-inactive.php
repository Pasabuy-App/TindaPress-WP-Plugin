<?php
	...
	*/
    ...
    class TP_Product_Store_Inactive {

        public static function listen(){
            return rest_ensure_response( 
                TP_Product_Store_Inactive:: list_store_inactive()
            );
        }
        ...
        public static function list_store_inactive(){
            ...
            $result = $wpdb->get_results("SELECT
                ...
                IF (( SELECT tp_rev.child_val FROM $table_revs tp_rev WHERE ID = tp_prod.status ) = 0, 'Inactive' , 'Active' ) AS `status`
            FROM
                $table_product tp_prod
            INNER JOIN 
                $table_revs tp_rev ON tp_rev.ID = tp_prod.title
